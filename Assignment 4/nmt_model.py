...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(self.hidden_size, embed_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.src(source_padded)
        print(X.shape)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, 0):
          ...
          dec_state, o_t, state = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
          o_prev = o_t
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        """
        src_sents_var = self.vocab.src.to_input_tensor([src_sent], device)
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_tm1 = torch.tensor([self.vocab.tgt[hyp[-1]] for hyp in hypotheses], dtype=torch.long, device=device)
