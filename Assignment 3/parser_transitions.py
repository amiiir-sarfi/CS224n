...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
      ...
      preds = model.predict(batch_parse)
      for j in range(len(preds)):
        batch_parse[j].parse([preds[j]])
        if len(batch_parse[j].buffer) == 0 and len(batch_parse[j].stack) == 1:
          unfinished_parses.remove(batch_parse[j])
